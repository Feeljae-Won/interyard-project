...
$(function(){
	...
	if(${vo.payWay == '카드'}){
		...
		payDetail = payDetail.substr(payDetail.indexOf('-'));
		payDetail += "-****-****-****";
	...
	$(document).on("click", ".dlvySelBtn" , function() {
		...
		let dlvyAddrNo = $(this).closest(".dlvyListItem").data("dlvyaddrno");
		...
		let confirmForm = `
			 	<form action="dlvyUpdate.do" method="post" id="confirmForm">
        			...
        			<input type="hidden" name="dlvyAddrNo" value="\${dlvyAddrNo}">
...
<div class="container">
	...
	<div class="border rounded p-3 mb-2 bg-white text-dark shadow-sm"> <!-- 배송지를 보여주는 div -->
		...
		<!-- 주소 -->
		${vo.addr } ${vo.addrDetail } (${vo.postNo })
		<br>
		<!-- 배송 메모 -->
