...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#dlvyList").load("/dlvy/list.do", function() {
			...
			if($listItem.length > 0) {
				//결제 폼에 기본 배송지 입력
				let dlvyaddrNo = $listItem.data("dlvyaddrno");
				let recipient = $listItem.find(".recipientModal").text();
				let tel = $listItem.find(".telModal").text();
				let addr = $listItem.find(".addrModal").text();
				
				$("#dlvyAddrNoPrint").val(dlvyaddrNo);
				$("#recipientPrint").text(recipient);
				$("#telPrint").text(tel);
				$("#addrPrint").text(addr);			
			} else {	
	...
	$(function() { //제이 쿼리 준비	
		...
		$("#orderSubmitBtn").click(function() {
			...
			if(isEmpty($("#dlvyAddrNoPrint"),"배송지",true) ){
				return false;
			}
			if(payWay=='카드') {
...
<div class="container" style="margin-bottom: 70px">
	...
	<form action="write.do" method="get" id="writeForm">	
		...
		<div class="border rounded p-3 mb-2 bg-white text-dark shadow-sm" id="dlvyPrintDiv">		
			...
			<input type="hidden" name="dlvyAddrNo" id="dlvyAddrNoPrint">
			<!-- 받는 사람(배송지명) -->
