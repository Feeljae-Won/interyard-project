...
public class OrderDAO extends DAO{
	...
	private String getWriteSql(OrderList list,OrderList optList) {		
		...
		for(OrderVO vo:list) {
			sql += " into orders(orderNo,dlvyAddrNo,dlvyMemo,orderPrice,dlvyCharge,payWay,payDetail,id,goodsNo,orderState,hopeDate) "
				+ " values((select max(orderNo)+"+(++x)+" from orders) ,"+vo.getDlvyAddrNo()+",'"+vo.getDlvyMemo()+"',"+vo.getOrderPrice()+","+vo.getDlvyCharge()+",'"+vo.getPayWay()+"','"+vo.getPayDetail()+"','"+vo.getId()+"','"+vo.getGoodsNo()+"','주문 확인','"+vo.getHopeDate()+"') ";
			while(y<optList.size()) {
				if(optList.get(y).getLefOrder()==vo.getOrder()) {
					...
					optSql += " into ordersOpt(orderOptNo,optNo,amount,orderNo) "
						+ " values((select max(orderOptNo)+"+(++y)+" from ordersOpt) ,"+opt.getOptNo()+","+opt.getAmount()+", (select max(orderNo)+"+(x-list.size())+" from orders)) ";
				} else break;
			}
		}		
	...
	private String getAdminTotalRowSQL(PageObject pageObj) {
		String sql = "select count(*) cnt from("
				...
				+ " o.dlvyAddrNo, d.recipient, d.tel, d.addr, d.postNo, o.payWay,o.payDetail from orders o, goods g, dlvyAddr d, category c "				
				+ " where 1=1 ";		
		sql += getSearch(pageObj)+getPeriod(pageObj);
		sql += " and (o.goodsNo = g.goodsNo) and (o.dlvyAddrNo = d.dlvyAddrNo ) and (g.categoryNo = c.categoryNo)) ";
		return sql;
	}
	...
	private String getAdminListSQL(PageObject pageObj) {
		String sql = " select  orderNo,goodsNo,orderState,hopeDate,orderDate,confirmDate,goodsTitle,goodsPublicher,goodsPrice,goodsCost,goodsStatus,categoryNo,categoryName,orderPrice,dlvyCharge,id,name,memberTel,dlvyAddrNo,recipient,tel,addr,postNo,payWay,payDetail  from("
				...
				+ " o.dlvyAddrNo, d.recipient, d.tel, d.addr, d.postNo, o.payWay,o.payDetail from orders o, goods g, dlvyAddr d, category c, member m "
				+ " where 1=1 " ;		
		sql += getSearch(pageObj)+getPeriod(pageObj);
		sql += " and (o.goodsNo = g.goodsNo) and (o.dlvyAddrNo = d.dlvyAddrNo ) and (g.categoryNo = c.categoryNo) and (o.id = m.id)"
	...
	private final String VIEW = "select o.orderNo, o.goodsNo, o.orderState, o.orderDate,o.confirmDate,o.payWay, o.payDetail,  g.goodsTitle, g.goodsImage,g.categoryNo ,o.orderPrice, o.dlvyCharge, o.id,"
			+ " oo.optNo, go.optName, oo.amount, o.dlvyAddrNo, to_char(o.hopeDate,'yyyy-mm-dd') hopeDate,d.dlvyName, d.recipient, d.tel, d.addr, d.postNo,o.dlvyMemo from orders o, goods g, goodsOption go, ordersOpt oo, dlvyAddr d "
			+ " where (o.id=? and o.orderNo=?) and (o.goodsNo = g.goodsNo) and (go.optNo = oo.optNo) and (o.orderNo = oo.orderNo) and (o.dlvyAddrNo = d.dlvyAddrNo )";
	
	//주문 상태를 수정하는 쿼리
	private final String STATEUPDATE = "update orders set orderState = ?, cancleReason=? where id=? and orderNo = ?";
	//배송지 정보를 바꾸는 쿼리
	private final String DLVYUPDATE = "update orders set dlvyAddrNo = ? where id=? and orderNo = ?";	
